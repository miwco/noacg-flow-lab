...
- Explicit Add state and Add transition commands open complete drafts in one stable Inspector. New transitions are visibly unsaved until created or canceled. Visible handles remain an optional faster connection method.
...
- The generic blank-flow renderer hides output in its initial OFF state, uses the first variable as its headline in every other state, and shows later variables as supporting data. Editable state-specific slot binding is not implemented yet.
